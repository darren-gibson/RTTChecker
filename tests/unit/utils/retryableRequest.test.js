import { shouldRetry, DEFAULT_RETRY_CONFIG, NetworkError, withRetry, fetchJsonWithRetry, calculateBackoffDelay } from '../../../src/utils/retryableRequest.js';
...
describe('retryableRequest utilities', () => {
  ...
  describe('withRetry', () => {
    ...
    test('throws last error after exhaustion', async () => {
      ...
      }, { maxRetries: 1, baseDelayMs: 1, maxDelayMs: 5 })).rejects.toThrow('Still bad');
    ...
    test('non-retryable status fails immediately (no retries)', async () => {
      ...
      }, { maxRetries: 3, baseDelayMs: 1, maxDelayMs: 5 })).rejects.toThrow('Auth');
    ...
    test('backoff delay respects jitter bounds (mocked Math.random)', async () => {
      ...
      try {
        ...
        const low = calculateBackoffDelay(1, { ...DEFAULT_RETRY_CONFIG, baseDelayMs: 100, maxDelayMs: 500 });
        Math.random = () => 1; // upper bound (30% jitter)
        const high = calculateBackoffDelay(1, { ...DEFAULT_RETRY_CONFIG, baseDelayMs: 100, maxDelayMs: 500 });
  ...
  describe('fetchJsonWithRetry', () => {
    test('retries on 503 then succeeds', async () => {
      ...
      const fetchImpl = jest.fn(() => {
        call++;
        if (call === 1) {
          return Promise.resolve({ ok: false, status: 503, statusText: 'Service Unavailable', text: () => Promise.resolve('down') });
        }
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ value: 42 }) });
      });
      const data = await fetchJsonWithRetry('http://x/test', { fetchImpl }, { maxRetries: 2, baseDelayMs: 1, maxDelayMs: 5 });
      expect(data).toEqual({ value: 42 });
      expect(fetchImpl).toHaveBeenCalledTimes(2);
    });
    test('fast fails on non-retryable status', async () => {
      const fetchImpl = jest.fn(() => Promise.resolve({ ok: false, status: 401, statusText: 'Unauthorized', text: () => Promise.resolve('bad creds') }));
      await expect(fetchJsonWithRetry('http://x/auth', { fetchImpl }, { maxRetries: 3 })).rejects.toThrow('HTTP request failed: 401 Unauthorized');
    ...
    test('wraps raw network error in NetworkError', async () => {
      ...
      await expect(fetchJsonWithRetry('http://x/net', { fetchImpl }, { maxRetries: 1, baseDelayMs: 1, maxDelayMs: 5 })).rejects.toThrow('ECONNRESET');
    ...
    test('custom buildError captures body and status', async () => {
      const fetchImpl = jest.fn(() => Promise.resolve({ ok: false, status: 502, statusText: 'Bad Gateway', text: () => Promise.resolve('temporary') }));
      const buildError = (res, body) => Object.assign(new Error(`Custom: ${res.status}`), { statusCode: res.status, responseBody: body });
      await expect(fetchJsonWithRetry('http://x/custom', { fetchImpl }, { buildError, maxRetries: 0 })).rejects.toThrow('Custom: 502');
    });
    test('merges headers from init and param', async () => {
      const fetchImpl = jest.fn(() => Promise.resolve({ ok: true, json: () => Promise.resolve({}) }));
      await fetchJsonWithRetry('http://x/h', { fetchImpl, init: { headers: { A: '1' } }, headers: { B: '2' } }, { maxRetries: 0 });
